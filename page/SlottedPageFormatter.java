...
public class SlottedPageFormatter {
    ...
    public static byte[] pageToBytes(SlottedPage page, TupleDesc td, int pageSize) {
        try {
            ...
            // write header and initialize variables
            ...
            for (int i = 0; i < numSlots; i++){
                if (page.isSlotUsed(i)){
                    Iterator<Field> fields= page.getTuple(i).fields();
            ...
            // write zeros in remaining space
            int curSize = dos.size();
            if (curSize < pageSize){
                int numZeros = pageSize - curSize;
                byte[] zerosToWrite= new byte[numZeros];
                dos.write(zerosToWrite,0,numZeros);
            }
    ...
    public static void bytesToPage(byte[] bytes, SlottedPage emptyPage, TupleDesc td) {
        ...
        try {
            ...
            // initializing size variables and reading in header
            ...
            // fetch fields from each tuple and write them to the empty page
            for (int i = 0; i < numSlots; i++){
                Tuple tupleToWrite = new Tuple(td);
                for (int j = 0; j < numFields; j++) {
                    tupleToWrite.setField(j, td.getFieldType(j).parse(dis));
                }
                if (isSlotUsed(i, header)){
                    emptyPage.insertTuple(i, tupleToWrite);
                }
            }
            dis.close();
        } catch (IOException e) {
    ...
    private static boolean isSlotUsed(int i, byte[] header) {
        ...
        byte slot = header[bytePos];
        return (slot & bitmask) > 0;
    }
    ...
    private static void markSlot(int i, byte[] header, boolean isUsed) {
        ...
        byte slot = header[bytePos];
        if (isUsed) {
            header[bytePos] = (byte)(slot | bitmask);
        }
        else {
            header[bytePos] = (byte)(slot & (~bitmask));
        }
